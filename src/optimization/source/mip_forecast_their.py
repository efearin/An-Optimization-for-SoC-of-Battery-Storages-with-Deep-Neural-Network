...
"""
Strategy, Scenario, Country = [True, False, False]
...
def Simulate(strategy=Strategy, ndays=1):
    ...
    if strategy:
        ...
        for i in range(ndays):
            L = L_ar[N*i:N*(i+1)].tolist() # Read daily load forecast
            PV = PV_ar[N*i:N*(i+1)].tolist() # Read daily PV forecast
        ...
        for i in range(ndays):
            ...
            for j in range(N):
                ...
                L = L_ar[N*i+j:N*(i+1)+j].tolist() # Read instantaneous load forecast
                PV = PV_ar[N*i+j:N*(i+1)+j].tolist() # Read instantaneous PV forecast
...
def Write(strategy=Strategy, ndays=1):
    if strategy:
        if Scenario:
            ...
            if Country:
                wlp.to_csv("../result/their_forecast/t_mip_battery_pv.csv")
            else:
                wlp.to_csv("../result/their_forecast/g_mip_battery_pv.csv")
        else:
            wlp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load_actual": L_actual_list[:N*ndays],
            ...
            if Country:
                wlp.to_csv("../result/their_forecast/t_mip_battery.csv")
            else:
                wlp.to_csv("../result/their_forecast/g_mip_battery.csv")
        wlp_cost = pd.DataFrame(data={"daily_cost": Daily_cost_list_LP}, columns=["daily_cost"])
        if Scenario:
            if Country:
                wlp_cost.to_csv("../result/their_forecast/t_mip_battery_pv_cost.csv")
            else:
                wlp_cost.to_csv("../result/their_forecast/g_mip_battery_pv_cost.csv")
        else:
            if Country:
                wlp_cost.to_csv("../result/their_forecast/t_mip_battery_cost.csv")
            else:
                wlp_cost.to_csv("../result/their_forecast/g_mip_battery_cost.csv")
    else:
        if Scenario:
            wdp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load_actual": L_actual_list[:N*ndays],
                ...
                "pv_actual", "pv_forecast", "grid", "feed", "battery-in", "battery-out", "energy", "cost"])
            if Country:
                wdp.to_csv("../result/their_forecast/t_dynamic_mip_battery_pv.csv")
            else:
                wdp.to_csv("../result/their_forecast/g_dynamic_mip_battery_pv.csv")
        else:
            wdp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load_actual": L_actual_list[:N*ndays],
                ...
                "load_forecast", "grid", "feed", "battery-in", "battery-out", "energy", "cost"])
            if Country:
                wdp.to_csv("../result/their_forecast/t_dynamic_mip_battery.csv")
            else:
                wdp.to_csv("../result/their_forecast/g_dynamic_mip_battery.csv")
        wdp_cost = pd.DataFrame(data={"daily_cost": Daily_cost_list_DP}, columns=["daily_cost"])
        if Scenario:
            if Country:
                wdp_cost.to_csv("../result/their_forecast/t_dynamic_mip_battery_pv_cost.csv")
            else:
                wdp_cost.to_csv("../result/their_forecast/g_dynamic_mip_battery_pv_cost.csv")
        else:
            if Country:
                wdp_cost.to_csv("../result/their_forecast/t_dynamic_mip_battery_cost.csv")
            else:
                wdp_cost.to_csv("../result/their_forecast/g_dynamic_mip_battery_cost.csv")
    return
