...
"""
Strategy, Scenario, Country = [True, True, False]
...
L_df = L_df[L_df.columns.values[5:101]]
L_ar = np.array(L_df.values)
L_ar = L_ar[96:34656] * 0.00001885071

PV_df = pd.read_csv("../data/pv.csv", header=0, index_col=0) #34560
PV_df = PV_df[PV_df.columns.values[5:101]]
PV_ar = np.array(PV_df.values)
if Scenario:
    PV_ar = PV_ar[0:34560] * 0.00036682367 # Battery with PV (PV included)
else:
    PV_ar = PV_ar[0:34560] * 0 # Battery only (PV not included)
...
def Write(strategy=Strategy, ndays=1):
    if strategy:
        if Scenario:
            wlp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load": L_actual_list[:N*ndays],
                "pv": PV_actual_list[:N*ndays], "grid": G_pos_list_LP, "feed": G_neg_list_LP, "battery-in": B_neg_list_LP,
                "battery-out": B_pos_list_LP, "energy": E_list_LP, "cost": Cost_list_LP}, columns=["date","from","to", "load",
                "pv", "grid", "feed", "battery-in", "battery-out", "energy", "cost"])
            if Country:
                wlp.to_csv("../result/optimal/t_mip_battery_pv.csv")
            else:
                wlp.to_csv("../result/optimal/g_mip_battery_pv.csv")
        else:
            wlp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load": L_actual_list[:N*ndays],
                "grid": G_pos_list_LP, "feed": G_neg_list_LP, "battery-in": B_neg_list_LP, "battery-out": B_pos_list_LP, "energy": E_list_LP,
                "cost": Cost_list_LP}, columns=["date","from","to", "load", "grid", "feed", "battery-in", "battery-out", "energy", "cost"])
            if Country:
                wlp.to_csv("../result/optimal/t_mip_battery.csv")
            else:
                wlp.to_csv("../result/optimal/g_mip_battery.csv")
        wlp_cost = pd.DataFrame(data={"daily_cost": Daily_cost_list_LP}, columns=["daily_cost"])
        if Scenario:
            if Country:
                wlp_cost.to_csv("../result/optimal/t_mip_battery_pv_cost.csv")
            else:
                wlp_cost.to_csv("../result/optimal/g_mip_battery_pv_cost.csv")
        else:
            if Country:
                wlp_cost.to_csv("../result/optimal/t_mip_battery_cost.csv")
            else:
                wlp_cost.to_csv("../result/optimal/g_mip_battery_cost.csv")
    else:
        if Scenario:
            wdp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load": L_actual_list[:N*ndays],
                "pv": PV_actual_list[:N*ndays], "grid": G_pos_list_DP, "feed": G_neg_list_DP, "battery-in": B_neg_list_DP,
                "battery-out": B_pos_list_DP, "energy": E_list_DP, "cost": Cost_list_DP}, columns=["date","from","to", "load",
                "pv", "grid", "feed", "battery-in", "battery-out", "energy", "cost"])
            if Country:
                wdp.to_csv("../result/optimal/t_dynamic_mip_battery_pv.csv")
            else:
                wdp.to_csv("../result/optimal/g_dynamic_mip_battery_pv.csv")
        else:
            wdp = pd.DataFrame(data={"date": Date[:N*ndays], "from": From[:N*ndays], "to": To[:N*ndays], "load": L_actual_list[:N*ndays],
                "grid": G_pos_list_DP, "feed": G_neg_list_DP, "battery-in": B_neg_list_DP, "battery-out": B_pos_list_DP, "energy": E_list_DP,
                "cost": Cost_list_DP}, columns=["date","from","to", "load", "grid", "feed", "battery-in", "battery-out", "energy", "cost"])
            if Country:
                wdp.to_csv("../result/optimal/t_dynamic_mip_battery.csv")
            else:
                wdp.to_csv("../result/optimal/g_dynamic_mip_battery.csv")
        wdp_cost = pd.DataFrame(data={"daily_cost": Daily_cost_list_DP}, columns=["daily_cost"])
        if Scenario:
            if Country:
                wdp_cost.to_csv("../result/optimal/t_dynamic_mip_battery_pv_cost.csv")
            else:
                wdp_cost.to_csv("../result/optimal/g_dynamic_mip_battery_pv_cost.csv")
        else:
            if Country:
                wdp_cost.to_csv("../result/optimal/t_dynamic_mip_battery_cost.csv")
            else:
                wdp_cost.to_csv("../result/optimal/g_dynamic_mip_battery_cost.csv")
    return
