...
df = pd.read_csv('../result/forecast/g_dynamic_mip_battery.csv', index_col=0)
...
df.to_csv('../result/adjusted/g_dynamic_mip_battery.csv')

#############

df = pd.read_csv('../result/adjusted/g_dynamic_mip_battery.csv', index_col=0)
costlist = list(map(float, pd.read_csv('../result/forecast/g_dynamic_mip_battery_cost.csv')['daily_cost']))
daily_cost = []
for x in range(360):
    daily_cost.append(df.iloc[96*x : 96*(x+1)]['adjusted_cost'].sum())
print(sum(costlist))
print(sum(daily_cost))
pd.DataFrame(data={"daily_cost": costlist, 'adjusted_daily_cost': daily_cost}, columns=["daily_cost",'adjusted_daily_cost']).reset_index(drop=True).to_csv('../result/adjusted/g_dynamic_mip_battery_cost.csv')
