...
"""
Strategy, Scenario, Country = [False, False, False]
...
G_pos, G_neg, B_pos, B_neg, E, s_pos, s_neg, b1, b2, b3 = [[0 for _ in range(N)], [0 for _ in range(N)],
    [0 for _ in range(N)], [0 for _ in range(N)], [0 for _ in range(N)], [0 for _ in range(N)],
    [0 for _ in range(N)], [False for _ in range(N)], [False for _ in range(N)], [False for _ in range(N)]]
...
def Optimize(p, PV, L, strategy=Strategy):
    global G_pos, G_neg, B_pos, B_neg, E, s_pos, s_neg, b1, b2, b3, E_init # To enable updating model decision variables and initial stored energy
    ...
    for n in range(N):
        ...
        s_pos[n] = m.addVar(lb=0, vtype=GRB.CONTINUOUS)
        s_neg[n] = m.addVar(lb=0, vtype=GRB.CONTINUOUS)
    ...
    for n in range(N):
        ...
        m.addConstr(s_pos[n] - s_neg[n] - PV[n] - B_pos[n] + B_neg[n] == 0)
        m.addConstr(s_pos[n] <= I_max * b3[n]) # Inverter efficiency reciprocity constraints (C4.1)
        m.addConstr(s_neg[n] <= I_max * (1-b3[n])) # Inverter efficiency reciprocity constraints (C4.2)
        m.addConstr(G_pos[n] - G_neg[n] - L[n] + eff * s_pos[n] - (1/eff) * s_neg[n] == 0)
